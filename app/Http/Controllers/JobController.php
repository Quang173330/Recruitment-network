...
class JobController extends Controller
{
    ...
    {
        $allJobs = Job::where('status', config('job_config.approve'))->with('images')->orderBy('created_at', 'desc')->paginate(5);
        ...
        foreach ($allJobs as $job) {
            $job->url =  $job->company->images()->first()->url;
        }
        if (Auth::check()) {
    ...
    public function edit($id)
    {
        ...
        if ($this->authorize('update', $job)) {
            ...
            $workingTimes = Tag::where('type', config('tag_config.working_time'))->get();
            $tags = $job->tags;
            return view('edit_job', compact('job','skills', 'langs', 'workingTimes', 'tags'));
        }
    }
