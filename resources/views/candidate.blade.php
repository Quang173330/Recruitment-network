                    ...
                    <div class="follow_jobs">
                        @foreach ($users as $user)
                            ...
                            <a href="{{ route('users.show', ['user' => $user]) }} ">
                                <img src="{{ asset($user->image->url) }}" alt="" class="img-circle" style="max-width:100px; max-height:140px;">
                                <div class="title my-title">
                                    ...
                                    <div>
                                        @if ($user->pivot->status == config('job_config.waiting'))
                                            <form action="{{ route('accept_reject', ['user_id' => $user->id, 'job_id' => $job->id, 'status' => config('job_config.accepted')]) }}" method="POST" style="margin-bottom: 10px">
                                                @csrf
                                                @method('patch')
                                            ...
                                            <form action="{{ route('accept_reject', ['user_id' => $user->id, 'job_id' => $job->id, 'status' => config('job_config.rejected')]) }}" method="POST">
                                                @csrf
                                                @method('patch')
                                                <button type="submit" class="btn btn-primary" style="width: 62.34px;"><p>@lang('job.reject')</p>
                                                </button>
                                            </form>
                                        @endif
                                    </div>
